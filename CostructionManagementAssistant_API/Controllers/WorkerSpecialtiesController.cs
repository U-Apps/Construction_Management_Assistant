...
namespace ConstructionManagementAssistant_API.Controllers
{
    ...
    public class WorkerSpecialtiesController(IUnitOfWork _unitOfWork) : ControllerBase
    {
        ...
        public async Task<ActionResult<BaseResponse<string>>> CreateWorkerSpecialty(AddWorkerSpecialtyDto request)
        {
            var result = await _unitOfWork.WorkerSpecialties.AddWorkerSpecialtyAsync(request);
            if (!result.Success)
        ...
        [ProducesResponseType(typeof(BaseResponse<string>), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(BaseResponse<string>), StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<BaseResponse<string>>> UpdateWorkerSpecialty(UpdateWorkerSpecialtyDto request)
        {
            var result = await _unitOfWork.WorkerSpecialties.UpdateWorkerSpecialtyAsync(request);
            if (!result.Success)
                return BadRequest(result);
            return Ok(result);
        }
