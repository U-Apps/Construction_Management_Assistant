...
namespace ConstructionManagementAssistant_API.Controllers
{
    ...
    public class WorkersController : ControllerBase
    {
        ...
        /// إنشاء عامل جديد
        /// </summary>
        /// <param name="worker">بيانات العامل</param>
        [HttpPost(SystemApiRouts.Worker.AddWorker)]
        [ProducesResponseType(typeof(BaseResponse<string>), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(BaseResponse<string>), StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<BaseResponse<string>>> CreateWorker(AddWorkerDto request)
        {
            var result = await _unitOfWork.Workers.AddWorkerAsync(request);
            if (!result.Success)
        ...
        /// تحديث عامل موجود
        /// </summary>
        /// <param name="Worker">بيانات العامل المحدثة</param>
        /// <returns>لا يوجد محتوى</returns>
        [HttpPut(SystemApiRouts.Worker.UpdateWorker)]
        [ProducesResponseType(typeof(BaseResponse<string>), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(BaseResponse<string>), StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<BaseResponse<string>>> UpdateWorker(UpdateWorkerDto request)
        {
            var result = await _unitOfWork.Workers.UpdateWorkerAsync(request);
            if (!result.Success)
                return BadRequest(result);
            return Ok(result);
        }
